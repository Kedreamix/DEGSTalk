...
def dilate_fn(bin_img, ksize=13):
    ...
    # 使用 maximum_filter 进行膨胀
    dilated_img = maximum_filter(padded_img, size=ksize, mode='constant')
    
    # 去掉填充部分，返回原图像大小的区域
    return dilated_img[pad:-pad, pad:-pad]

def erode_fn(bin_img, ksize=13):
    """
    对图像进行腐蚀操作。
    ...
    # 使用 minimum_filter 进行腐蚀
    eroded_img = minimum_filter(padded_img, size=ksize, mode='constant')
    
    # 去掉填充部分，返回原图像大小的区域
    return eroded_img[pad:-pad, pad:-pad]


def dilate_fn_torch(bin_img, ksize=13):
    """
    使用 PyTorch 对图像进行膨胀操作。
    ...
    - 处理后的图像 (numpy 数组)
    """
    # 确保输入是 PyTorch 张量，并且具有 batch 和 channel 维度
    if isinstance(bin_img, np.ndarray):
        bin_img = torch.from_numpy(bin_img).float().unsqueeze(0).unsqueeze(0)
    
    # 计算填充大小
    pad = (ksize - 1) // 2
    
    # 使用 reflect 模式进行填充
    bin_img_padded = F.pad(bin_img, pad=[pad, pad, pad, pad], mode='reflect')
    
    # 进行最大池化作为膨胀操作
    dilated_img = F.max_pool2d(bin_img_padded, kernel_size=ksize, stride=1, padding=0)
    
    # 去掉额外的维度，并转换回 numpy 数组
    dilated_img = dilated_img.squeeze().numpy()
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".jpg", audio_file='', audio_extractor='deepspeech'):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames)):
            ...
            bg = image * ~( erode_fn(talking_dict['face_only_mask']+talking_dict['mouth_mask']) + talking_dict['mouth_mask']+talking_dict['neck_mask']+talking_dict['cloth_mask'])[:,:,np.newaxis]
            torso_img_path = os.path.join(path, 'torso_imgs', str(frame['img_id']) + '.png')
            torso_img = np.array(Image.open(torso_img_path).convert("RGBA")) * 1.0
